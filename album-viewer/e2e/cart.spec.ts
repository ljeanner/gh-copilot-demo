...
test.describe('Cart Screenshots', () => {
  test('take screenshot of album grid with cart icon', async ({ page }) => {
    await page.goto('/')
    await page.waitForSelector('.album-card')
    
    await page.screenshot({ path: 'e2e/screenshots/album-grid-with-cart.png', fullPage: true })
  })

  test('take screenshot of cart with items', async ({ page }) => {
    await page.goto('/')
    await page.waitForSelector('.album-card')
    
    // Add items to cart
    await page.locator('.album-card').nth(0).locator('button:has-text("Add to Cart")').click()
    await page.locator('.album-card').nth(1).locator('button:has-text("Add to Cart")').click()
    ...
    await page.screenshot({ path: 'e2e/screenshots/cart-panel-with-items.png', fullPage: true })
  })

  test('take screenshot of empty cart', async ({ page }) => {
    await page.goto('/')
    await page.waitForSelector('.album-card')
    ...
    await page.screenshot({ path: 'e2e/screenshots/cart-panel-empty.png', fullPage: true })
  })

  test('take screenshot of album card in cart state', async ({ page }) => {
    await page.goto('/')
    await page.waitForSelector('.album-card')
    
    // Add first album to cart
    await page.locator('.album-card').first().locator('button:has-text("Add to Cart")').click()
    await page.waitForSelector('.btn-in-cart')
    
    await page.screenshot({ path: 'e2e/screenshots/album-card-in-cart-state.png', fullPage: true })
  })
})
